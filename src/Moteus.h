...
public:
    ...
    CanFdFrame MakeQuery(const mm::Query::Format *format_override = nullptr)
    {
        return MakeFrame(mm::EmptyMode(), {}, {},
                         format_override == nullptr ? &options_.query_format : format_override);
    }
                            ...
                            const mm::Query::Format *query_override = nullptr)
    {
        return MakeFrame(mm::PositionMode(),
                         cmd,
                         (command_override == nullptr ? mm::PositionMode::Format() : *command_override),
                         query_override);
    }
                        ...
                        const mm::Query::Format *query_override = nullptr)
    {
        return MakeFrame(mm::VFOCMode(),
                         cmd,
                         (command_override == nullptr ? mm::VFOCMode::Format() : *command_override),
                         query_override);
    }
                           ...
                           const mm::Query::Format *query_override = nullptr)
    {
        return MakeFrame(mm::CurrentMode(),
                         cmd,
                         (command_override == nullptr ? mm::CurrentMode::Format() : *command_override),
                         query_override);
    }
                              ...
                              const mm::Query::Format *query_override = nullptr)
    {
        return MakeFrame(mm::StayWithinMode(),
                         cmd,
                         (command_override == nullptr ? mm::StayWithinMode::Format() : *command_override),
                         query_override);
    }
                                 ...
                                 const mm::Query::Format *query_override = nullptr)
    {
        return MakeFrame(mm::OutputNearest(),
                         cmd,
                         (command_override == nullptr ? mm::OutputNearest::Format() : *command_override),
                         query_override);
    }
                               ...
                               const mm::Query::Format *query_override = nullptr)
    {
        return MakeFrame(mm::OutputExact(),
                         cmd,
                         (command_override == nullptr ? mm::OutputExact::Format() : *command_override),
                         query_override);
    }
                                  ...
                                  const mm::Query::Format *query_override = nullptr)
    {
        return MakeFrame(mm::RequireReindex(),
                         cmd,
                         (command_override == nullptr ? mm::RequireReindex::Format() : *command_override),
                         query_override);
    }
                                             ...
                                             const mm::Query::Format *query_override = nullptr)
    {
        return MakeFrame(mm::RecapturePositionVelocity(),
                         cmd,
                         (command_override == nullptr ? mm::RecapturePositionVelocity::Format() : *command_override),
                         query_override);
    }
    ...
    bool Poll()
    {
        ...
        CANFDMessage rx_msg;
        if (!ACAN_T4::can3.receiveFD(rx_msg))
        {
            return false;
        }
    ...
    bool BeginSingleCommand(const mm::CanFdFrame &frame)
    {
        ...
        const bool desired_brs =
            (frame.brs == CanFdFrame::kDefault ? !options_.disable_brs : frame.brs == CanFdFrame::kForceOn ? true
                                                                                                           : false);
...
private:
                         ...
                         const mm::Query::Format *query_override = nullptr)
    {
        auto result = DefaultFrame(
            query_override != nullptr ? kReplyRequired : options_.default_query ? kReplyRequired
                                                                                : kNoReply);
